...
function FLHARecordCard({ record, employees, currentEmployee, canCreate, sharedSigner, setSharedSigner, onEdit, onReassess, onSaved, onError }: { record: FieldRecord; employees: Employee[]; currentEmployee?: Employee; canCreate: boolean; sharedSigner: string; setSharedSigner: (value: string) => void; onEdit: () => void; onReassess: () => void; onSaved: () => Promise<void>; onError: (value: string | null) => void }) {
  ...
  async function sign(employee: Employee, supervised = false) {
    if (supervised && !window.confirm(`Pass the device to ${employeeName(employee)}. ${employeeName(employee)}, confirm that you reviewed this FLHA, understand its controls, and will stop work if conditions change.`)) return;
    try {
      await fieldOperationsApi.signRecord(record.id, {
        employee_id: employee.id,
        employee_name: employeeName(employee),
        acknowledgement: "I reviewed this FLHA, understand the hazards and controls, and will stop work if conditions change.",
        supervised_shared_device: supervised,
        worker_confirmation: supervised,
        supervisor_confirmation: supervised ? "I supervised this worker reviewing and personally confirming their acknowledgement on the shared device." : null,
      });
      await onSaved();
