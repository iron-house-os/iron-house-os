...
export function AppLayout({ children }: PropsWithChildren) {
  ...
  return (
    <div className="min-h-screen bg-iron-50 text-iron-950">
      <aside
        className={`fixed inset-y-0 left-0 z-30 flex w-72 flex-col border-r border-iron-100 bg-white transition-transform lg:translate-x-0 ${
      ...
      {isSidebarOpen ? (
        <button
          ...
          className="fixed inset-0 z-20 bg-iron-950/30 lg:hidden"
          onClick={() => setIsSidebarOpen(false)}
        />
      ) : null}
