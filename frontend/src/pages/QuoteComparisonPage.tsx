            ...
            <div key={index} className="rounded-md border border-iron-100 bg-white p-4">
              ...
              <div className="grid gap-3 md:grid-cols-2 xl:grid-cols-4">
                ...
                <label className="grid gap-1 text-sm">
                  ...
                  <select
                    value={quote.scope_type}
                    onChange={(event) => updateQuote(index, { scope_type: event.target.value as SupplierQuoteCreate["scope_type"] })}
                    className="rounded-md border border-iron-100 px-3 py-2"
                  >
                ...
                <label className="flex items-end gap-2 pb-2 text-sm font-medium text-iron-700">
                  <input
                    ...
                    onChange={(event) => updateQuote(index, { is_selected: event.target.checked })}
                    className="h-4 w-4"
                  />
                  Selected quote
                </label>
                ...
                {result.lines.map((line, index) => (
                  <tr key={`${line.line_item_code ?? "line"}-${line.scope}-${index}`} className="border-t border-iron-100">
                    ...
                    <td className="px-3 py-2 text-iron-600">
                      {line.lowest_supplier} - {moneyFormatter.format(line.lowest_amount ?? 0)}
                    </td>
                    <td className="px-3 py-2 text-iron-600">
