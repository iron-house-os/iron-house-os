...
export function EstimatingPage() {
  ...
  const payload = useMemo<EstimateCreate>(
    () => ({
      ...
      risks: [
        {
          description: "Unknown utilities / site conditions",
          amount: riskAmount,
      ...
      <div className="flex flex-col gap-4 border-b border-iron-100 pb-6 lg:flex-row lg:items-end lg:justify-between">
        <div>
          ...
          <p className="mt-2 max-w-3xl text-sm leading-6 text-iron-500">
            Build a first-pass Iron House estimate using production defaults, vendor quotes, disposal, risk, markups, and workbook export.
          </p>
        </div>
          ...
          <div className="rounded-xl border border-iron-100 bg-white p-5 shadow-sm">
            <div className="flex items-center justify-between gap-3">
              <div>
                ...
                <p className="text-sm text-iron-500">Use production defaults, direct unit costs, or selected vendor quotes.</p>
              </div>
              <button type="button" onClick={addLineItem} className="inline-flex items-center gap-2 rounded-md border border-iron-100 px-3 py-2 text-sm font-medium">
        ...
        <aside className="space-y-6">
          <div className="rounded-xl border border-iron-100 bg-white p-5 shadow-sm">
            ...
            <div className="mt-4 grid gap-3">
              ...
              <NumberField label="Risk probability %" value={riskProbability} onChange={setRiskProbability} />
        ...
        <>
          ...
          <div className="mt-5 border-t border-iron-100 pt-4">
            ...
            <dl className="mt-3 space-y-2 text-sm">
              <SummaryRow label="Labour" value={summary.category_breakdown.labour} />
              <SummaryRow label="Equipment" value={summary.category_breakdown.equipment} />
              <SummaryRow label="Material" value={summary.category_breakdown.material} />
              <SummaryRow label="Disposal" value={summary.category_breakdown.disposal} />
              <SummaryRow label="Subcontract" value={summary.category_breakdown.subcontract} />
              <SummaryRow label="Indirect" value={summary.category_breakdown.indirect} />
              <SummaryRow label="Risk" value={summary.category_breakdown.risk} />
            </dl>
          </div>
        </>
      ) : (
...
function NumberField({ label, value, onChange }: { label: string; value: number; onChange: (value: number) => void }) {
  return (
    <label className="flex items-center justify-between gap-3 text-sm font-medium text-iron-700">
      {label}
      <input type="number" value={value} onChange={(event) => onChange(Number(event.target.value))} className="w-28 rounded-md border border-iron-100 px-3 py-2 text-right text-sm" />
    </label>
  );
}
