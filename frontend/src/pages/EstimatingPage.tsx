...
export function EstimatingPage() {
  const [projectName, setProjectName] = useState("Marine Drive Parking Lot");
  const [projectCode, setProjectCode] = useState("WR26-012");
      ...
      <div className="flex flex-col gap-4 border-b border-iron-100 pb-6 lg:flex-row lg:items-end lg:justify-between">
        <div>
          ...
          <p className="mt-2 max-w-3xl text-sm leading-6 text-iron-500">
            Build a first-pass Iron House estimate using production activity defaults, markups, risk,
            indirects, and workbook export.
          </p>
        </div>
        <div className="flex flex-wrap gap-2">
          <button
            ...
            className="inline-flex items-center gap-2 rounded-md bg-iron-950 px-3 py-2 text-sm font-medium text-white"
          >
        ...
        <div className="space-y-6">
          ...
          <div className="rounded-xl border border-iron-100 bg-white p-5 shadow-sm">
            <div className="flex items-center justify-between gap-3">
              <div>
                <h2 className="text-lg font-semibold text-iron-950">Line Items</h2>
                <p className="text-sm text-iron-500">
                  Choose a production activity or enter direct unit cost for quote/manual pricing.
                </p>
              </div>
              <button
                type="button"
                onClick={addLineItem}
                className="inline-flex items-center gap-2 rounded-md border border-iron-100 px-3 py-2 text-sm font-medium"
              >
              ...
              {lineItems.map((item, index) => (
                <div key={`${item.code}-${index}`} className="rounded-lg border border-iron-100 p-4">
                  <div className="grid gap-3 md:grid-cols-[120px_1fr_120px_120px]">
                    <input
                      value={item.code ?? ""}
                      onChange={(event) => updateLineItem(index, { code: event.target.value })}
                    ...
                    <select
                      value={item.unit}
                      onChange={(event) => updateLineItem(index, { unit: event.target.value as EstimateUnit })}
                      className="rounded-md border border-iron-100 px-3 py-2 text-sm"
                    >
                      {(["LS", "EA", "m", "m2", "m3", "t", "hr", "day"] as EstimateUnit[]).map(
                        (unit) => (
                  ...
                  <div className="mt-3 grid gap-3 md:grid-cols-[1fr_160px_120px]">
                    <select
                      value={item.default_activity ?? ""}
                      onChange={(event) =>
                        updateLineItem(index, {
                          default_activity: event.target.value
                            ? (event.target.value as DefaultProductionActivity)
                            : null,
                        })
                      }
                      className="rounded-md border border-iron-100 px-3 py-2 text-sm"
                    >
                      ...
                      {rateLibrary.map((rate) => (
                        <option key={rate.activity} value={rate.activity}>
                          {activityLabels[rate.activity]} — {rate.production_rate_per_hour}/hr
                        </option>
                      ))}
                    </select>
                    <input
                      ...
                      onChange={(event) =>
                        updateLineItem(index, {
                          direct_unit_cost: event.target.value === "" ? null : Number(event.target.value),
                        })
                      }
                      className="rounded-md border border-iron-100 px-3 py-2 text-sm"
                      placeholder="Direct $/unit"
                    />
                    <button
                      type="button"
                      onClick={() => removeLineItem(index)}
                      className="inline-flex items-center justify-center gap-2 rounded-md border border-iron-100 px-3 py-2 text-sm text-red-700"
                    >
        ...
        <aside className="space-y-6">
          <div className="rounded-xl border border-iron-100 bg-white p-5 shadow-sm">
            ...
            <div className="mt-4 grid gap-3">
              ...
              <NumberField label="Contingency %" value={contingency} onChange={setContingency} />
              <NumberField label="Overhead %" value={overhead} onChange={setOverhead} />
              <NumberField label="Profit %" value={profit} onChange={setProfit} />
              <NumberField label="Bonding %" value={bonding} onChange={setBonding} />
          ...
          <div className="rounded-xl border border-iron-100 bg-white p-5 shadow-sm">
            <div className="flex items-center justify-between">
              <h2 className="text-lg font-semibold text-iron-950">Summary</h2>
              {isCalculating || isLoadingRates ? <RefreshCw className="h-4 w-4 animate-spin" /> : null}
            </div>
            {summary ? (
              <dl className="mt-4 space-y-3 text-sm">
                <SummaryRow label="Direct" value={summary.direct_cost} />
                <SummaryRow label="Indirect" value={summary.indirect_cost} />
                <SummaryRow label="Risk" value={summary.risk_cost} />
                <SummaryRow label="Contingency" value={summary.contingency} />
                <SummaryRow label="Overhead" value={summary.overhead} />
                <SummaryRow label="Profit" value={summary.profit} />
                <div className="border-t border-iron-100 pt-3">
                  <SummaryRow label="Final Bid" value={summary.final_price} strong />
                </div>
                <div className="flex justify-between text-iron-500">
                  <dt>Gross margin</dt>
                  <dd>{summary.gross_margin_percent.toFixed(2)}%</dd>
                </div>
              </dl>
            ) : (
              <p className="mt-4 text-sm text-iron-500">Calculate the estimate to view bid totals.</p>
            )}
          </div>
        </aside>
      </form>
...
function NumberField({
  ...
  return (
    <label className="flex items-center justify-between gap-3 text-sm font-medium text-iron-700">
      {label}
      <input
        type="number"
        value={value}
        onChange={(event) => onChange(Number(event.target.value))}
        className="w-28 rounded-md border border-iron-100 px-3 py-2 text-right text-sm"
      />
    </label>
  );
}
...
function SummaryRow({ label, value, strong = false }: { label: string; value: number; strong?: boolean }) {
  return (
    <div className={`flex justify-between ${strong ? "text-base font-semibold text-iron-950" : "text-iron-700"}`}>
      <dt>{label}</dt>
