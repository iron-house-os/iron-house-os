import { Bot, Brain, MessageSquareText, Send, ShieldCheck, Upload } from "lucide-react";
    ...
    <section className="space-y-6">
      ...
      <div className="rounded-md border border-iron-100 bg-white shadow-sm">
        <div className="flex items-center justify-between border-b border-iron-100 p-4">
          ...
          <span className="text-xs font-medium text-iron-500">Typed, read-only assistant</span>
        </div>
        <div className="min-h-[360px] space-y-4 p-4" aria-live="polite">
          {messages.length === 0 ? <div className="grid min-h-[300px] place-items-center text-center"><div><MessageSquareText className="mx-auto h-8 w-8 text-brand-gold" /><p className="mt-3 font-semibold">Ask how to use Iron House OS</p><p className="mt-1 text-sm text-iron-500">Try “Where do I review project costs?”</p></div></div> : messages.map((message) => <div key={message.id} className={`max-w-3xl rounded-md p-3 text-sm leading-6 ${message.role === "user" ? "ml-auto bg-brand-gold text-brand-black" : "bg-iron-50 text-iron-800"}`}><div className="mb-1 text-xs font-semibold uppercase tracking-wide opacity-70">{message.role === "user" ? "Management" : "Iron House Chat"}</div>{message.content}</div>)}
        </div>
      ...
      <p className="text-xs text-iron-500">Iron House Chat accepts typed questions only. Do not enter passwords, SINs, banking, medical, or payroll information.</p>
    </section>
  );
}
