...
export type CustomerInvoice = { id: string; invoice_number: string; project_id: string | null; project_name: string; site_address: string | null; customer_name: string; customer_address: string; customer_phone: string | null; invoice_date: string; due_date: string; terms: string; status: "draft" | "approved" | "issued" | "paid" | "void"; line_items: Array<{ description: string; quantity: string; unit_price: string; amount: string }>; subtotal: string; gst_rate: string; gst: string; total: string; development_seed_key: string | null };
...
export const financeApi = {
  getCustomerInvoices: () => request<{ items: CustomerInvoice[]; total: number }>("/finance/customer-invoices"),
  createCustomerInvoice: (payload: Record<string, unknown>) => request<CustomerInvoice>("/finance/customer-invoices", { method: "POST", body: JSON.stringify(payload) }),
  updateCustomerInvoiceStatus: (id: string, status: CustomerInvoice["status"]) => request<CustomerInvoice>(`/finance/customer-invoices/${id}/status`, { method: "PATCH", body: JSON.stringify({ status }) }),
