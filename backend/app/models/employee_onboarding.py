...
class EmployeeOnboarding(UUIDPrimaryKeyMixin, TimestampMixin, Base):
    ...
    encrypted_invitation_token: Mapped[str | None] = mapped_column(Text)
    ...
    invitation_delivery_status: Mapped[str] = mapped_column(String(40), nullable=False, default="not_generated")
    invitation_delivery_attempted_at: Mapped[datetime | None] = mapped_column(DateTime(timezone=True))
    invitation_delivered_at: Mapped[datetime | None] = mapped_column(DateTime(timezone=True))
    invitation_delivery_error_code: Mapped[str | None] = mapped_column(String(80))
