...
The project workspace gives management and estimators one read-only launch view after a project is awarded. The dashboard connects the generated job number and project-start checklist to the estimate, baseline budget, purchase-order requests, safety records, field-production posting, and project documents.
...
- blocked safety-launch status, requirement count, internal folder status, portal-access status, and assignment count;
- controlled field-production posting status and blocker codes, daily-sheet and posted-record counts, latest sheet status, and internal Field Production folder status; and
...
The field-production indicator is also derived. It reports `blocked` until the controlled safety release, portal assignment, and complete mobilization checklist are all ready. Counts come from project-bound daily sheets and their typed posting manifests; the dashboard does not create or approve a field record.
